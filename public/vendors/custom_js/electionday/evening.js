...
function getGenderChart() {
    ...
    // var party_representatives_able_to_observe_overall = echarts.init(document.getElementById('party_representatives_able_to_observe'));
    // var ballot_paper_stuffing_occured_overall = echarts.init(document.getElementById('ballot_paper_stuffing_occured'));
    var ballot_paper_counted_at_station_overall = echarts.init(document.getElementById('ballot_paper_counted_at_station'));
    var adequate_lighting_overall = echarts.init(document.getElementById('adequate_lighting'));
    var ballot_boxes_transported_offsite_overall = echarts.init(document.getElementById('ballot_boxes_transported_offsite'));
    ...
    var ballot_paper_supply_overall = echarts.init(document.getElementById('ballot_paper_supply'));
    ...
    voting_resources_options = barTemplate({"ballot papers":1200, "ballot boxes":100, "Embossig devices":500, "Ink bottles":1000, pens:590, "Voter List": 780});
    sealed_ballot_papers_options = pieTemplate(["Yes", "No"], "Sealed ballot papers", dashboardData.ballot_boxes_guarded);
    show_voter_registration_certificate_options = pieTemplate(["Yes", "No"], "Allowed", dashboardData.show_voter_registration_certificate);
    ...
    party_representatives_able_to_observe_options = pieTemplate(["Yes", "No"], "Voters given privacy", dashboardData.party_representatives_able_to_observe);
    ballot_paper_stuffing_occured_options = pieTemplate(["Yes", "No"], "Voters given privacy", dashboardData.ballot_paper_stuffing_occured);
    ballot_paper_counted_at_station_options = pieTemplate(["Yes", "No"], "Ballot papers counted at the polling station", dashboardData.ballot_paper_counted_at_station);
    adequate_lighting_options = pieTemplate(["Yes", "No"], "Adequate lighting during ballot counting", dashboardData.adequate_lighting);
    ballot_boxes_transported_offsite_options = pieTemplate(["Yes", "No"], "Ballot boxes transported somewhere else", dashboardData.ballot_boxes_transported_offsite);
    ...
    incidents_occured_options = barTemplate({"Quarelling": 50, "Fighting": 38, "non-regulatory": 64, "Safety": 45, "medical emergencies": 12, "Intimidation": 48});
    injuries_occured_options = barTemplate({"Fainted/Collapsed": 25, "died": 2, "injured": 15});
    ballot_paper_supply_options = barTemplate({"Initial supply": 200000, "Additional supply": 50000, "Unused": 2000, "Spoiled": 2000, "voided": 12000, "total supply": 250000});
    ...
    // party_representatives_able_to_observe_overall.setOption(party_representatives_able_to_observe_options);
    // ballot_paper_stuffing_occured_overall.setOption(ballot_paper_stuffing_occured_options);
    ballot_paper_counted_at_station_overall.setOption(ballot_paper_counted_at_station_options);
    adequate_lighting_overall.setOption(adequate_lighting_options);
    ballot_boxes_transported_offsite_overall.setOption(ballot_boxes_transported_offsite_options);
    ...
    ballot_paper_supply_overall.setOption(ballot_paper_supply_options);
